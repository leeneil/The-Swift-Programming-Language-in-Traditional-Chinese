...
新的命名好的列舉，結構，類和協議類型。你也可以使用一條聲明來延長一個已經存在的命名好​​的類型的行為。或者在你的
程序裡引入在其他地方聲明的符號。
...
你可以在全局，函數體內或者類，結構體，列舉，擴展聲明的上下文中使用這種形式的聲明。
當變量以這種形式在全局或者一個函數內被聲明時，它代表一個計算型變量。當他在類，結構體，列舉，擴展聲明的上下文
中中被聲明時，他代表一個計算型變量屬性。
...
當他在類，結構體，列舉，擴展聲明的上下文中被聲明時，監視器代表屬性監視器。
...
你可以使用函數聲明在你的程序裡引入新的函數。函數可以在類的上下文，結構體，列舉，或者作為方法的協議中被聲明。
...
以self修飾的列舉或結構體方法必須以mutating關鍵字作為函數聲明頭。

子類重寫的方法必須以override關鍵字作為函數聲明頭。不用override關鍵字重寫的方法，使用了override關鍵字
卻並沒有重寫父類方法都會報錯。

和類型相關而不是和類型實例相關的方法必須在static聲明的結構以或列舉內，亦或是以class關鍵字定義的類內。
...
在你的程序裡使用列舉聲明來引入一個列舉類型。

列舉聲明有兩種基本的形式，使用關鍵字enum來聲明。列舉聲明體使用從零開始的變量——叫做列舉事件，和任意數量的
聲明，包括計算型屬性，實例方法，靜態方法，構造器，類型別名，甚至其他列舉，結構體，和類。列舉聲明不能
包含析構器或者協議聲明。

不像類或者結構體。列舉類型並不提供隱式的初始構造器，所有構造器必須顯式的聲明。構造器可以委託列舉中的其他
構造器，但是構造過程僅當構造器將一個列舉時間完成後才全部完成。

和結構體類似但是和類不同，列舉是值類型：列舉實例在賦予變量或常量時，或者被函數調用時被複製。
更多關於值類型的信息，參見結構體和列舉都是值類型(Structures and Enumerations Are Value Types)一節。

你可以擴展列舉類型，正如在擴展名聲明(Extension Declaration)中討論的一樣。

###任意事件類型的列舉

如下的形式聲明了一個包含任意類型列舉時間的列舉變量
...
這種形式的列舉聲明在其他語​​言中有時被叫做可識別聯合(discrinminated)。
...
事件不同，這種形式的列舉事件包含一個同類型的基礎值，叫做原始值(raw value)。這些值的類型在原始值類型(raw value type)
...
列舉事件的初始值可以調用方法roRaw獲得，如ExampleEnum.B.toRaw()。你也可以通過調用fromRaw方法來使用初始值找到
其對應的事件，並返回一個可選的事件。查看更多信息和獲取初始值類型事件的信息，參閱初始值(raw values​​)。

###獲得列舉事件

使用點(.)來引用列舉類型的事件，如EnumerationType.EnumerationCase。當列舉類型可以上下文推斷出時，你可以
...
列舉類型是模式匹配(pattern-matched)的，和其相反的是switch語句case塊中列舉事件匹配，在列舉事件類型(Enumeration Case Pattern)中有描述。
...
類型別名，甚至其他結構體，類，和列舉聲明。結構體聲明不能包含析構器或者協議聲明。詳細討論和包含多種結構體
聲明的實例，參見類和結構體一節。

結構體可以包含任意數量的協議，但是不能繼承自類，列舉或者其他結構體。
...
結構體和列舉都是值類型(Structures and Enumerations Are Value Types)一節。
...
類型別名，甚至其他結構體，類，和列舉聲明。類聲明不能包含協議聲明。詳細討論和包含多種類聲明的實例，參見類和
結構體一節。
...
構造器聲明會為程序內的類，結構體或列舉引入構造器。構造器使用關鍵字Init來聲明，遵循兩條基本形式。

結構體，列舉，類可以有任意數量的構造器，但是類的構造器的規則和行為是不一樣的。不像結構體和列舉那樣，類
有兩種結構體，designed initializers 和convenience initializers，參見構造器一節。

如下的形式聲明了結構體，列舉和類的指定構造器：
...
結構體和列舉的構造器可以帶哦用其他的已聲明的構造器，來委託其中一個火全部進行初始化過程。
...
擴展聲明用於擴展一個現存的類，結構體，列舉的行為。擴展聲明以關鍵字extension開始，遵循如下的規則：
...
附屬腳本聲明，甚至其他結構體，類，和列舉聲明。擴展聲明不能包含析構器，協議聲明，存儲型屬性，屬性監測器或其他
的擴展屬性。詳細討論和查看包含多種擴展聲明的實例，參見擴展一節。

擴展聲明可以向現存的類，結構體，列舉內添加一致的協議。擴展聲明不能向一個類中添加繼承的類，因此
...
附屬腳本聲明只能在類，結構體，列舉，擴展和協議聲明的上下文進行聲​​明。
