...
Swift 語言存在兩種類型：命名型類型和復合型類型。 *命名型類型*是指定義時可以給定名字的類型。命名型類型包括類、結構體、列舉和協議。比如，一個用戶定義的類`MyClass`的實例擁有類型`MyClass`。除了用戶定義的命名型類型，Swift 標準庫也定義了很多常用的命名型類型，包括那些表示陣列、字典和可選值的類型。
...
類型`Optional<T>`是一個列舉，有兩種形式，`None`和`Some(T)`，又來代表可能出現或可能不出現的值。任意類型都可以被顯式的聲明（或隱式的轉換）為可選類型。當聲明一個可選類型時，確保使用括號給`?`提供合適的作用範圍。比如說，聲明一個整型的可選陣列，應寫作`(Int[])?`，寫成`Int[]?`的話則會出錯。
...
##元類型
元類型是指所有類型的類型，包括類、結構體、列舉和協議。

類、結構體或列舉類型的元類型是相應的類型名緊跟`.Type`。協議類型的元類型——並不是運行時適配該協議的具體類型——是該協議名字緊跟`.Protocol`。比如，類`SomeClass`的元類型就是`SomeClass.Type`，協議`SomeProtocol`的元類型就是`SomeProtocal.Protocol`。
...
列舉定義中的類型繼承子句可以是一個協議列表，或是指定原始值的列舉，一個單獨的指定原始值類型的命名型類型。使用類型繼承子句來指定原始值類型的列舉定義的例子，見章節“原始值”。
